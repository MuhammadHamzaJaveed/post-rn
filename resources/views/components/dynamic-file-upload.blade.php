<div x-data="{
        ...
        validateFileSize(event) {
                ...
                const allowedImageTypes = ['image/jpeg', 'image/jpg'];
                    ...
                    if (file) {
                        ...
                        } else if (event.target.accept.includes('.jpg') || event.target.accept.includes('.jpeg')) {
                            if (!allowedImageTypes.includes(file.type)) { // Image format validation
                                this.errorMessage = 'Only JPG or JPEG images are allowed.';
                                event.target.value = '';
                            } else {
                                this.errorMessage = '';
    ...
    <!-- If no photo is uploaded or the update form is triggered -->
    ...
    <div style="margin-left: -30%" x-show="errorMessage" x-cloak
        ...
        The uploaded file must be 1 MB or smaller and only jpg or jpeg files are accepted.
    </div>
    ...
    <div class="text-center error text-sm text-red-600 mt-2">
        The uploaded file should be less than 1MB and not empty.
    </div>
    @enderror
</div>