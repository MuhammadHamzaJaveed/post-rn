...
class GenerateExcelReportPref extends Command
{
    protected $signature = 'report:generate-excel-pref';
    ...
    public function handle()
    {
        ...
        $filename = 'excel/bds-private-report-pref-' . Carbon::now()->format('Y-m-d_H-i-s') . '.xlsx';
        ...
        Mail::mailer('verification')
            ...
            ->send(new ReportGeneratedMail($filename,'pref'));
        Mail::mailer('verification')
    ...
    private function mapRecordData($record): array
    {
        ...
        if ($record?->seat?->id == 1 || $record?->seat?->id == 3)
        {
            $openMerit = 'Yes';
        }
        elseif ($record?->seat?->id == 2)
        {
            $openMerit = 'No';
        }
        else{
    ...
    {
        $preferences = [];
        $collegePreferences = $record->bdsCollegePreferences->pluck('college_pref')->first();
