...
class GenerateExcelReportPrefOverseas extends Command
{
    protected $signature = 'report:generate-excel-pref-overseas';
    protected $description = 'Generate an Excel report with prefferences';
    ...
    public function handle()
    {
        ...
        $filename = 'excel/bds-private-seat-both-foreigner-pref' . Carbon::now()->format('Y-m-d_H-i-s') . '.xlsx';
        ...
        Mail::mailer('verification')
            ...
            ->send(new ReportGeneratedMail($filename,'pref_overseas'));
        Mail::mailer('verification')
            ->to('mumar1052@gmail.com')
            ->send(new ReportGeneratedMail($filename,'pref_overseas'));
    ...
    private function getBaseColumns(): array
    {
        return [
            ...
            'Foreigner' => 'foreigner',
            'Foreigner Preferences' => 'is_open_merit',
            'Comments' => 'comments',
    ...
    {
        $preferences = [];
        $collegePreferences = $record->bdsCollegeOpenForeigner?->pluck('college_pref')?->first();
