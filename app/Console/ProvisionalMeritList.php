...
class ProvisionalMeritList extends Command
{
    protected $signature = 'promeritlistgen';
    /**
...
//    }
    ...
    {
        $kuch = User::with([
            'personalDetails' => function ($query) {
            $query->select( 'mother_name', 'cnic_passport', 'gender_id', 'date_of_birth','district_id','nationality_id'); 
        },

        // 'qualifications' => function ($query) {
        //     $query->select('ssc_marks_obtained', 'ssc_total_marks', 'ssc_passing_year', 'hssc_marks_obtained', 'hssc_total_marks','hssc_passing_year'); 
        // },
        // 'admissionTest' => function ($query) {
        //     $query->select('md_cat_obtained_marks', 'mcat_obtained_marks'); 
        // }

        ])->select('id', 'name', 'father_name', 'program_id', 'program_priority', 'aggregate' , 'comments', 'status')->limit(7)->get();

        dd (User::with('personalDetails')->toSql());
    }
